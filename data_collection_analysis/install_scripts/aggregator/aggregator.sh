...
function config_init {
  ...
  if [ -z "$APP_CONTAINER_REPO" ]; then
    tput setaf 1
    echo "" 1>&2
    echo $'ERROR: Please update Application Container Repo  information in the configuration file '
    tput sgr0
    exit 1
  fi
  if [ -z "$OPA_CONTAINER_REPO" ]; then
    tput setaf 1
    echo "" 1>&2
    echo $'ERROR: Please update the OPA Container Repo  information in the configuration file '
    tput sgr0
    exit 1
  fi
